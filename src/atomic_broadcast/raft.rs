...
fn create_rawraft_config(pid: u64, seed: u64) -> Config {
    ...
    let tick_period = 100;
    /*config["raft"]["tick_period"]
        .as_i64()
        .expect("Failed to load tick_period") as usize;*/
    let leader_hb_period = 100;
    //config["raft"]["leader_hb_period"]
    //    .as_i64()
    //    .expect("Failed to load leader_hb_period") as usize;
    let max_batch_size = 50000000;
    /*config["raft"]["max_batch_size"]
        .as_i64()
        .expect("Failed to load max_batch_size") as u64;*/
    let pre_vote = false;
    /*config["raft"]["pre_vote"]
        .as_bool()
        .expect("Failed to load pre_vote");*/
    let check_quorum = false;
    /*config["raft"]["check_quorum"]
        .as_bool()
    ...
    S: RaftStorage + Send + Clone + 'static,
{
    pub fn with(pid: u64, initial_config: Vec<u64>, reconfig_policy: ReconfigurationPolicy, seed: u64, logger: Logger) -> Self {
        ...
        let raw_raft =
            RawNode::new(&raw_raft_config, store).expect("Failed to create tikv Raft").with_logger(&logger);
    ...
    fn on_ready(&mut self) -> Handled {
        ...
        //println!("READY MESSAGES: {}!", ready_msgs.len());
        for msg in ready_msgs {
            ...
            let mut type_string = "type: ".white().bold(); 

            if msg_type == MessageType::MsgRequestVote || msg_type == MessageType::MsgRequestVoteResponse {
                type_string = "type: ".yellow().bold();
            }
            ...
            /*if msg.get_msg_type() == MessageType::MsgAppend && rand::random::<u8>() < 10 {
                receiver
                    .tell_serialised(RaftMsg(msg.clone()), self)
    ...
    fn tick(&mut self) -> Handled {
        ...
        //println!("TICKKKKKKKKK count: {}, id: {}", self.timer_tick_count, self.raft_replica.raw_raft.raft.id);
        ...
        if leader != 0 {
            ...
            if leader != self.current_leader {
                info!(self.logger, "New leader: {}, old: {}, term: {}", leader, self.current_leader, self.raft_replica.raw_raft.raft.term);
    ...
    fn propose(&mut self, proposal: Proposal) {
        ...
        match proposal.reconfig {
            ...
            None => {
                ...
                //println!("{} In raft. PID: {} Count: {}", "PROPOSING: ".bold().purple(), self.pid, self.proposal_count);
    ...
    fn raft_replica_receive_local_raftreplicamsg_stop(&mut self) {
        ...
        //COMMUNICATOR LOGIC
        debug!(self.logger, "Sending stop to {:?}", self.peers.keys());
    ...
    fn raft_comp_receive_local_raftcompmsg_leader(&mut self, notify_client: bool, pid: u64) {
        ...
        debug!(self.logger, "Node {} became leader. Received by {}", pid, self.pid);
        //self.became_leader_count += 1;
        ...
        match m.data.ser_id {
            ...
            _ => {
                ...
                match_deser! {data {
                    msg(p): PartitioningActorMsg [using PartitioningActorSer] => {
                        match p {
                            PartitioningActorMsg::Init(init) => {
                                ...
                                info!(self.logger, "Raft got init, pid: {}", init.pid);
                    ...
                    err(e) => error!(self.logger, "Error deserialising msg: {:?}", e),
